 ...
 */
function cmmc_add_excerpts_to_pages() {
  add_post_type_support( 'page', 'excerpt' );
}
add_action( 'init', 'cmmc_add_excerpts_to_pages' );

/**
 * Add class to excerpt
 */
function cmmc_add_class_to_excerpt( $excerpt ) {
  return str_replace( '<p', '<p class="post-excerpt"', $excerpt );
}
add_filter( 'the_excerpt', 'cmmc_add_class_to_excerpt' );

/**
 * Add next and prev classes to previous/next post links
 */
function cmmc_add_class_prev_link( $class ) {
  return str_replace( '<a', '<a class="prev-post"', $class );
}
add_filter( 'previous_post_link', 'cmmc_add_class_prev_link' );

function cmmc_add_class_next_link( $class ) {
  return str_replace( '<a', '<a class="next-post"', $class );
}
add_filter( 'next_post_link', 'cmmc_add_class_next_link' );
 ...
 */
function cmmc_excerpt_ellipse( $text ) {
  return str_replace( array( '[...]', '[&hellip;]' ), '&hellip;', $text );
}
add_filter( 'get_the_excerpt', 'cmmc_excerpt_ellipse' );
